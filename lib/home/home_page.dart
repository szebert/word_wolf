  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<AppBloc, AppState>(
      listenWhen: (previous, current) =>
          previous.completedGamesCount != current.completedGamesCount,
      listener: (context, state) {
        // Check if the user has played enough games to prompt for review
        context.read<InAppReviewBloc>().add(
              InAppReviewCheck(
      ...
      child: BlocListener<InAppReviewBloc, InAppReviewState>(
        listenWhen: (previous, current) =>
            previous.status != current.status &&
            current.status == InAppReviewStatus.available,
        listener: (context, state) {
          // Request review when it becomes available
          context.read<InAppReviewBloc>().add(
          ...
          appBar: AppBar(
            actions: [
              AppIconButton(
                ...
                onPressed: () =>
                    Navigator.of(context).push(AIConfigPage.route()),
